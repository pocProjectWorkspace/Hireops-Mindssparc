...
89. **Button primary's brand-500 default fails axe AA color-contrast (3.67:1 vs 4.5:1).** Discovered by the CRS-01 axe scan on the apply form. The submit button is overridden per-call via `className="bg-brand-600 hover:bg-brand-700 active:bg-brand-700"` to clear the axe threshold (brand-600 = 5.2:1, brand-700 = 7.4:1). The /triage axe scan passes only because the visible default state has no primary brand-500 buttons. Fixing the variant default everywhere is a wider design-system change tracked in open-questions.md; apply the same per-call override for any new primary buttons on candidate-facing surfaces until then.
