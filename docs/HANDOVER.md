...
61. **`db:seed:demo-candidates` provisions 3 demo applications in kyndryl-poc.** Required for the Module 1b Playwright drawer-open test (it skips if no demo data). One Maya (fresh + high score), one Rohan (48h-old → SLA breach in Hot Zone), one Alex (fresh + mid score). Idempotent — re-runs delete-then-reinsert the application rows so `stage_entered_at` refreshes (so Rohan stays "48h old" each run). Demo-day rehearsal also wants this seed.
