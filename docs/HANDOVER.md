...
9. **Fixed role enum, no custom roles.** Wave 1 uses `public.tenant_role` with 11 fixed values (`admin`, `recruiter`, `hiring_manager`, `panel_member`, `hr_ops`, `people_ops`, `it_admin`, `partner_admin`, `partner_user`, `candidate`, `employee`). Custom tenant-defined roles are deferred to Wave 2+. The enum is defined in `0004_db01_identity.sql` and mirrored in TypeScript at `packages/db/src/schema/roles.ts` (`TENANT_ROLES` const + `TenantRole` type). The auth hook reads from `tenant_user_memberships.roles` (now `tenant_role[]`), so the JWT `roles` claim is implicitly constrained by the enum.
10. **Drizzle still types `tenant_user_memberships.roles` as `text[]`.** The SQL column is `tenant_role[]` but the Drizzle schema keeps `text[]` because `drizzle-orm`'s `pgEnum` support is unreliable. Practical impact: code inserting into memberships via Drizzle (`db.insert(tenantUserMemberships).values({ roles: ['admin'] })`) will fail with "text[] cannot be assigned to tenant_role[]". Workaround pattern is in `packages/db/src/test-fixtures/setup-test-user.ts`: bypass Drizzle and use raw SQL via the `sql` pool client with an explicit `::tenant_role[]` cast. Revisit if Drizzle's enum support improves.
11. **Drizzle snapshot chain is broken from 0001/0002.** `pnpm db:generate` refuses to run because the 0001 and 0002 snapshot metadata files share an `id` and `prevId`. DB-01 worked around this by hand-writing `0004_db01_identity.sql` instead of using generate. Any future migration that wants `db:generate` needs to first repair the snapshot chain (regenerate 0001/0002/0003 snapshots so each has a unique id and proper prevId pointer). The actual DB state and SQL files are correct; only the drizzle-kit metadata is wrong.
