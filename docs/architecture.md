...
5. **Per-tenant data residency** is acknowledged as a future need (region-per-tenant for jurisdictions that mandate local data). The data model accommodates it (a tenant's region is a tenant attribute that drives storage/compute placement), but Wave 1 ships single-region. Development environment runs in ap-northeast-1 (Tokyo) on Supabase shared pooler; production POC tenant deploys to ap-south-1 (Mumbai) per data residency requirements (DPDPA-aligned). Multi-region tenant placement is Wave 3+ and will be revisited in the production roadmap.
