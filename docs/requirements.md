...
| **Data residency** | India data may need to stay in Indian data centres (DPDPA does not currently require it but Kyndryl policy may). Architectural implication: production POC tenant deploys to Supabase region ap-south-1 / Mumbai. **Development environment runs in ap-northeast-1 (Tokyo)** for cost and Supabase-tier availability during the build phase; cutover to ap-south-1 happens before any real candidate data lands. |
