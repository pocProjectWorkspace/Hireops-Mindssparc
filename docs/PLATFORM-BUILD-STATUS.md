...
**Last updated:** 18 August 2026 (main @ `e338bd7`). Latest work — **partner portal completed P0–P2 and the reporting module built R0–R1** — is the "Update 12–18 August" block below; its full handoff is `docs/new-set/HANDOFF-18aug-partner-and-reports.md`. The dated update blocks in this section run oldest → newest, and there is a recorded gap between 17 July and 12 August (see the note at the end of the August block).

*Historic header, kept for context (14 July 2026, post-demo, main @ `bf01008`):* ALL demo-critical build work is DONE — §4 items 1–10 complete, CI green (first fully-green board 13 July, run 29243695378), STAGING IS LIVE, and a four-phase UI/UX revamp (DESIGN-01→04) shipped: token system, app shell, ten shared primitives, every internal + candidate surface restyled, zero behavioral change.
...
**Update 16–17 July 2026 (main @ `f66148f` + local): PARTNER SHIPPED + HOSTED, WAVE A CLOSED, WAVE B UNDERWAY.**
- **PARTNER-01/02:** partner portal (new app `apps/partner-portal`, port 3005 dev) hosted at **https://hireops-partner-portal.vercel.app** (own Vercel project, Root Directory apps/partner-portal, CLI-deployed — NOT git-connected); partner auth tier (`partnerProcedure`, partner_users resolution, org-isolated reads); candidate submission with wireflows dedup/ownership (90d claim, race-guarded) feeding the SAME recruiter pipeline (`partner_empanelled` source). Partner demo login: partner1@talentbridge-partners.test / TestPassword123! (org TalentBridge Partners, seed `db:seed:partner-demo`, a6xx namespace, groom-safe).
...
**Update 12–18 August 2026 (main @ `e338bd7`): PARTNER PORTAL COMPLETED (P0–P2) + REPORTING MODULE BUILT (R0–R1).**
Full detail: **`docs/new-set/HANDOFF-18aug-partner-and-reports.md`** — read that before touching either area; it carries the commit table, the pinned measure definitions, and the operational traps. Summary:

- **Why partner first.** The client asked for reporting + a notetaker. Assessing that PDF against the codebase (`docs/new-set/reporting-notetaker-build-plan.md`) surfaced that the partner portal was ~80% unbuilt against its own documented scope and **not operable** — partner orgs/users/assignments could only be created by `db:seed:partner-demo`, `partner_invitations` was dormant, and the ownership-claim expiry sweep its schema declares load-bearing had never been built, so **an expired claim blocked resubmission of that candidate forever**. Plan + audit: `docs/new-set/partner-portal-build-plan.md`.
- **Partner P0 (`e0fe248`→`8310f26`, pushed + deployed 13 Aug):** internal `/partners` admin surface (empanel / invite / assign, admin+hr_ops), the accept-invite flow, the `ownership_claim_sweep` worker + claim release, three partner emails (stage + date + candidate name only, §6.3, enforced by a payload allowlist test), migration **0113** attribution FKs, recruiter-side "Partner · Acme" attribution + dedup dispute read, and hardening (Vercel env guard, worker 23505-by-code fix).
- **Partner P1 (`51288c5`→`8d0694d`, 15 Aug):** `/reqs` list + detail (JD, comp band, knockouts), `/submissions` list + detail (ownership banner, stage-and-date-only timeline), org-admin team management + needs-attention feed, password reset + invitation revoke.
- **Partner P2 (`28c1db3`, `5f56813`, 15 Aug):** migration **0114** `partner_msa` + `partner_fees` (close-and-reopen terms versioning; fees freeze their terms in `msa_snapshot`), MSA editor, fee accrual on offer-accept, the claim window now reading the live MSA, and the partner `/commercials` tab. **This unblocked reports #7 and #8.** Partner P3 (messaging + scanner, bulk, talent pool, ad-hoc email intake) stays gated.
- **Reporting R0 (`4e80344`→`fc00db3`, pushed 16 Aug):** the **semantic layer** (`apps/api/src/lib/reports/`, `packages/api-types/src/reports.ts`) so no two reports compute a measure differently — `getRecruitmentReport` was refactored onto it with an old-vs-new differential proving identical numbers; then the `/reports` catalog (admin/hr_head/hr_ops, one shared filter bar), requisition aging, recruiter productivity, pipeline & speed + SLA hot spots, admin-only governance cards, CSV export and role-gated drill-downs.
- **Reporting R1 (`5c742f9`→`e338bd7`, 17–18 Aug):** headcount vs plan (incl. reqs raised outside any envelope), approval cycle analytics (per-step approver attribution), the partner scorecard, migration **0115** `interviews.completed_at`/`cancelled_at` (+ the API now stamping them), interview & scorecard health, onboarding readiness, and the **executive summary / board pack** — composed from the reports that own each number, with print styling instead of PDF export.
- **State at handoff:** pushed through `fc00db3`; **5 commits unpushed** (`5c742f9`, `a024529`, `0b9c6b6`, `deb4dc0`, `e338bd7`). Migrations 0113/0114/0115 are applied to staging and verified. Railway still needs its manual `railway up` after any push touching apps/api, apps/workers or shared packages.
- **Next:** R1.5a (scheduled digest backend — designed, nothing written yet) then R1.5b (its admin UI). Then the deliberately gated work: **diversity capture (#13/#14) needs a legal/product decision — there are zero demographic fields in the schema**, and the **notetaker** phase is heavier than the PDF implies (no inbound webhooks exist anywhere, storage has no signed URLs, consent is one-shot at apply time).

> **Docs gap, stated honestly:** this file was last updated 17 July and the block above jumps to 12 August. Work between those dates (Iris assistant, layout standardisation, admin surfaces, branding, the demo-fix round) is recorded in commit messages and in `docs/new-set/HANDOFF-31jul-iris-tester-feedback.md`, not here.
