...
/**
 * Per-route error boundary for /admin/workflows server-component failures
 * (e.g. a listAgents rejection). Reset re-renders the segment.
 */
  ...
  return (
    <main className="mx-auto mt-12 max-w-prose rounded-md border border-status-error-500 bg-status-error-50 p-6">
      <h1 className="mb-2 text-xl font-semibold text-status-error-700">
        Couldn&apos;t load agent workflows
      </h1>
      <p className="mb-4 text-sm text-neutral-700">
        We hit an error fetching the agent list. Try again, or sign out and back in if it persists.
      </p>
      <button
        type="button"
        onClick={() => reset()}
